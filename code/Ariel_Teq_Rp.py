...
Temperature - Radius - Spectral Type
'''
...
Ariel_low_plot = ax.scatter(ariel_ESM_low["Planet Temperature [K]"], ariel_ESM_low['Planet Radius [Rj]'],
                        alpha=0.4, s = 50, c = "pink", marker="o",
                        label = "Ariel", zorder = 1)
Ariel_ESM_plot = ax.scatter(ariel_ESM_high["Planet Temperature [K]"], ariel_ESM_high['Planet Radius [Rj]'],
...
plt.legend(handles=legend_elements, title='Spectral Type',   loc='lower right')


plt.grid(True, alpha=0.35)
plt.xlabel("Planetary Equilibrium Temperature [K]", fontsize=18)
plt.ylabel("Planet Radius [$R_J$]", fontsize=18)
plt.title("Ariel Phase Curve Targets: Temperature vs Radius", fontsize=24)
#plt.yscale('log')
# plt.ylim([0,105])
plt.savefig(save_dir + 'Ariel-PhaseCurve-Temp-Rad.jpg')
